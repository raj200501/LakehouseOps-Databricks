...
def test_admin_demo_start_returns_run_id():
    c = TestClient(app)
    response = c.post("/admin/demo/start")
...
def test_admin_demo_start_is_idempotent():
    c = TestClient(app)
    first = c.post("/admin/demo/start")
    second = c.post("/admin/demo/start")
    assert first.status_code == 200
    assert second.status_code == 200
    assert first.json()["run_id"] > 0
    assert second.json()["run_id"] > 0
